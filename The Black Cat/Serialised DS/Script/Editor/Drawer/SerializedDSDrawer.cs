...
namespace TheBlackCat.SerialisedDS.Editor
{
    ...
    public class SerializedPriorityQueueDrawer : SerialisedDSDrawer
    {
        protected override RLDrawer CreateRL(SerializedProperty property, GUIContent label)
        {
            return new PriorityQueueRL(property, label, fieldInfo);
        }
    }

    [CustomPropertyDrawer(typeof(SerializedHashSet<>), true)]
    public class SerializedHashSetDrawer : SerialisedDSDrawer
    {
        protected override RLDrawer CreateRL(SerializedProperty property, GUIContent label)
        {
            return new HashSetRL(property, label, fieldInfo);
        }
    }
}